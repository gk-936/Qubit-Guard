...
def run_http_test(name, url, method="GET", body=None):
    print(f"\n{'='*60}")
    print(f"TEST: {name}")
    print(f"{'='*60}")
    try:
...
data = run_http_test("PQC Algorithm Registry", f"{BASE}/pqc/algorithms")
if data:
...
# Test 2: Audit Table
data = run_http_test("PQC Verification Audit", f"{BASE}/pqc/audit")
if data:
    print(f"Count: {data['count']}")
...
# Test 3: ML Selector
data = run_http_test("ML Selector (Web/Server)", f"{BASE}/pqc/select", "POST", {
...
data = run_http_test("ML Selector (Mobile/App)", f"{BASE}/pqc/select", "POST", {
...
data = run_http_test("ML Selector (Archival/HSM)", f"{BASE}/pqc/select", "POST", {
...
# Test 6: Health check
data = run_http_test("Health Check", f"{BASE}/health")
if data:
