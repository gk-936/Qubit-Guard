...
| 🆕 | API pillar showed nothing (no message at all) when the API target was unreachable, unlike every other pillar | ✅ Fixed — added an explicit "API Endpoint Unreachable" finding |
| 🆕 | **Major**: 4 of 5 pillars (`_scan_vpn_tls`/`_scan_api_jwt`/`_scan_firmware`/`_scan_archival`) fabricated a fallback QVS (75/90/75/75) instead of `None` whenever nothing was actually measured — only `_scan_web_tls` was correctly fixed earlier. The overall-QVS aggregation's "exclude unassessed pillars" logic only ever worked for Web in practice; every scan with an unreachable VPN/API/Firmware/Archival target silently pulled the overall score toward ~75-90 instead of correctly excluding it | ✅ Fixed all four to match `_scan_web_tls`'s pattern. Verified live: unreachable vpn+api target → `riskScores = {web: 85, vpn: None, api: None, firmware: 90, archival: 85, overall: 87}` (correctly averages only the 3 measured pillars); dashboard confirmed handling real (not synthetic) `None` values correctly for the first time |
